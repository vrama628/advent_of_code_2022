...
module HashKey = struct
  type t = (int * int) * int * phase [@@deriving sexp_of]
...
let () =
  ...
  in
  let queue = Queue.singleton (start, valley, 0, First) in
  let hash_key_of ~pos ~time ~phase = pos, time % n_unique_valleys, phase in
  let seen = Hash_set.create (module HashKey) in
  Hash_set.add seen (hash_key_of ~pos:start ~time:0 ~phase:First);
  let rec bfs () =
    let pos, valley, time, phase = Queue.dequeue_exn queue in
    if Float.(Random.float 1. < 0.01) then
      printf "\r%s %d%!" (show_phase phase) (Hash_set.length seen);
    Hash_set.add seen (hash_key_of ~pos ~time ~phase);
    if equal_phase phase Third && Poly.(pos = end_) then
      time
    else
      let phase =
        match phase with
        | First when Poly.(pos = end_) -> Second
        | Second when Poly.(pos = start) -> Third
        | other -> other
      in
      ...
      |> List.filter ~f:(fun pos ->
             ...
             && not
                  (Hash_set.mem seen (hash_key_of ~pos ~time:(time + 1) ~phase))
         )
      |> List.map ~f:(fun pos ->
             Hash_set.add seen (hash_key_of ~pos ~time:(time + 1) ~phase);
             pos, valley, time + 1, phase
         )
