...
let () =
  ...
  in
  let rec pour sand =
    let not_blocked point = not (Set.mem rock point || Set.mem sand point) in
    let rec pour_unit (i, j) =
      if j > abyss then
        None
      else if not_blocked (i, j + 1) then
        pour_unit (i, j + 1)
    ...
    | Some point -> pour (Set.add sand point)
    | None -> Set.length sand
  in
  printf "%d\n" (pour empty)